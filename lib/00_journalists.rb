...
def perform
    ...
    puts ">>>> Il y a #{handle.select{|word|word.length == 6 }.count} handle contenant 5 caractères."
    puts ">>>>> Il y a #{handle.select{|word|word=~/^@[A-Z]/}.count} handle dont le nom commence par une majuscule."
    puts ">>>>>> Voulez-vous trier les handle par ordre alphabétique ? oui/non"
    handle = is_alpha(handle)
    puts ">>>>>>> Les handle ont été trié par ordre alphabétique"
    puts ">>>>>>>> Voulez-vous trier les handle par ordre croissant? oui/non"
    handle = is_length(handle)
    puts ">>>>>>>>> Les handle ont été trié par ordre croissant"
